...
export const processDirectory = (route) => {
  ...
  files.forEach((content) => {
    ...
    const isDirectory = fs.statSync(absolutePathFile).isDirectory();
    if (isDirectory) {
    ...
    } else if (path.extname(content) === ".md") {
      pathFile.push(absolutePathFile); // Si es un archivo .md, lo agregamos al array
    }
  });
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(route, "utf8", (err, data) => {
      ...
      renderer.link = (href, title, text) => {
        ...
        links.push({ href, text: truncatedText, file: route });
      };
